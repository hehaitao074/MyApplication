...
public class JianDanGridAdapter extends BaseAdapter <JianDanBean.CommentsBean>{

    public JianDanGridAdapter(Context mContext, List<JianDanBean.CommentsBean> datas) {
    ...
    public void onBindViewHolder( RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof MyViewHolder) {
            JianDanBean.CommentsBean resultsBean = datas.get(position);
            //加载图片
            ImageOptions imageOptions = new ImageOptions.Builder().setRadius(2).setUseMemCache(true).setFadeIn(true).setSize(120, 160).build();
            x.image().bind(((MyViewHolder) holder).img, resultsBean.getPics().get(0), imageOptions);
